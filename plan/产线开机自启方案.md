...
安装 [`deploy/install-kiosk.sh`](../deploy/install-kiosk.sh) 后，可通过 Web UI 菜单或 `POST /api/system/mode` 在 **prod** 与 **dev** 间切换；实际配置由 root helper [`deploy/mode-switch.sh`](../deploy/mode-switch.sh) 写入并重启生效。验收步骤见 [`deploy/mode-switch-verify.md`](../deploy/mode-switch-verify.md)。

| 维度 | 生产模式 (prod) | 开发模式 (dev) |
|------|-----------------|----------------|
| 自动登录 | `markeye` | `ubuntu` |
| 后端 | `markeye-web.service` (systemd) | XDG autostart → [`deploy/dev-web.sh`](../deploy/dev-web.sh)（非 systemd） |
| 浏览器 | Chromium kiosk 全屏 | Chrome/Chromium 普通窗口 → [`deploy/dev-browser.sh`](../deploy/dev-browser.sh) |
| kiosk 加固 | 启用 | 禁用 |

```mermaid
flowchart TB
  subgraph prodMode [生产模式]
    pBoot[开机] --> pLogin[自动登录 markeye]
    pLogin --> pSystemd[markeye-web.service]
    pLogin --> pKiosk[markeye-kiosk.desktop]
    pSystemd --> pApi[FastAPI :8080]
    pKiosk --> pFull[Chromium kiosk全屏]
  end

  subgraph devMode [开发模式]
    dBoot[开机] --> dLogin[自动登录 ubuntu]
    dLogin --> dWeb[markeye-dev-web.desktop]
    dLogin --> dBrowser[markeye-dev-browser.desktop]
    dWeb --> dServer[dev-web.sh 后台启动]
    dBrowser --> dChrome[Chrome 普通窗口]
    dServer --> dApi[FastAPI :8080]
    dChrome --> dUi["/template/"]
  end
```
...
| [`deploy/mode-switch.sh`](../deploy/mode-switch.sh) | prod/dev 模式切换（root helper） |
| [`deploy/dev-web.sh`](../deploy/dev-web.sh) | 开发模式后端自启（非 systemd） |
| [`deploy/dev-browser.sh`](../deploy/dev-browser.sh) | 开发模式普通浏览器自启 |
| [`deploy/markeye-dev-web.desktop`](../deploy/markeye-dev-web.desktop) | 开发模式后端 XDG 模板 |
| [`deploy/markeye-dev-browser.desktop`](../deploy/markeye-dev-browser.desktop) | 开发模式浏览器 XDG 模板 |
| [`deploy/mode-switch-verify.md`](../deploy/mode-switch-verify.md) | 模式切换验收步骤 |
